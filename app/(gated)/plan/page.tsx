...
export default async function PlanPage({
  ...
  const selectedStrength = strengthByDate.get(selectedDate) ?? null;
  const selectedLog = loggedByDate.get(selectedDate);

  const [strengthDone, strengthLog] = selectedDay
    ? await Promise.all([checkedExercises(selectedDate), strengthLogFor(selectedDate)])
    : [new Set<string>(), null];
          ...
          <section className="block block--tight">
            <div className="row-between" style={{ marginBottom: "0.75rem" }}>
              <div>
                ...
                <p className="tile__value" style={{ marginTop: "0.25rem" }}>
                  {dayTitle(selectedDay, selectedStrength ?? undefined)}
                </p>
              </div>
            ...
            <div className="stack">
              <SessionCard
                workout={selectedDay}
                log={selectedLog}
                hasStrength={selectedStrength !== null}
              />
              {selectedStrength ? (
                <StrengthCard
                  session={selectedStrength}
                  done={strengthDone}
                  log={strengthLog}
                />
              ) : null}
